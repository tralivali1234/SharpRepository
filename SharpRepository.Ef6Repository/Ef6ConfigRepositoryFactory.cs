...
{
    public class Ef6ConfigRepositoryFactory : ConfigRepositoryFactory
    {
        public Ef6ConfigRepositoryFactory(IRepositoryConfiguration config)
            : base(config)
        {
        }
        ...
        {
            return new Ef6Repository<T>(GetDbContext());
        }

        public override IRepository<T, TKey> GetInstance<T, TKey>()
        {
            return new Ef6Repository<T, TKey>(GetDbContext());
        }

        public override ICompoundKeyRepository<T, TKey, TKey2> GetInstance<T, TKey, TKey2>()
        {
            return new Ef6Repository<T, TKey, TKey2>(GetDbContext());
        }
        ...
        private DbContext GetDbContext()
        {
            ...
            {
                throw new ConfigurationErrorsException("The connectionString attribute is required in order to use the Ef6Repository via the configuration file, unless you set the RepositoryDependencyResolver to use an Ioc container.");
            }
